...
int main(int argc, char **argv) 
{
  ...
  MPI_Win_create(mem, 2*size*sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &grid_win);
  ...
  for(int iter=0; iter<niters; ++iter) {
    ...
    MPI_Put(&aold[ind(bx,1)], 1, east_west_type, east, ind(bx+1,1)+offset, 1, east_west_type, grid_win);
    MPI_Put(&aold[ind(1,1)], 1, east_west_type, west, ind(0,1)+offset, 1, east_west_type, grid_win);
    MPI_Win_fence(0, grid_win);
