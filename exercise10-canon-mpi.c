...
int main ( int argc , char **argv )
{
	...
	for (int i = 0; i < TILE_WIDTH; i++) {
		...
		MPI_Send(tileA, TILE_WIDTH, MPI_FLOAT, left, 0, comm_cart);
		MPI_Send(tileB, TILE_WIDTH, MPI_FLOAT, up, 0, comm_cart);
		MPI_Recv(tileA, TILE_WIDTH, MPI_FLOAT, right, 0, comm_cart, &status);
		MPI_Recv(tileB, TILE_WIDTH, MPI_FLOAT, down, 0, comm_cart, &status);
	}
